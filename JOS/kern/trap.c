...
void trap_init(void) {
  ...
  SETGATE(
      idt[0], 0, GD_KT, th0,
      0); //格式如下：SETGATE(gate, istrap, sel, off, dpl)，定义在inc/mmu.h中
  ...
  SETGATE(
      idt[T_SYSCALL], 0, GD_KT, th_syscall,
      3); //为什么门的DPL要定义为3，参考《x86汇编语言-从实模式到保护模式》p345
...
void page_fault_handler(struct Trapframe *tf) {
  ...
  if (curenv->env_pgfault_upcall) {
    ...
    utr->utf_esp =
        tf->tf_esp; // UXSTACKTOP栈上需要保存发生缺页异常时的%esp和%eip
