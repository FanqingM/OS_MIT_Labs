...
static void load_icode(struct Env *e, uint8_t *binary) {
  ...
  lcr3(PADDR(
      e->env_pgdir)); //这步别忘了，虽然到目前为止e->env_pgdir和kern_pgdir除了PDX(UVPT)这一项不同，其他都一样。
  ...
  for (int i = 0; i < ph_num; i++) {
    if (ph[i].p_type == ELF_PROG_LOAD) { //只加载LOAD类型的Segment
      ...
      memset(
          (void *)ph[i].p_va, 0,
          ph[i].p_memsz); //因为这里需要访问刚分配的内存，所以之前需要切换页目录
      memcpy((void *)ph[i].p_va, binary + ph[i].p_offset,
             ph[i].p_filesz); //应该有如下关系：ph->p_filesz <=
                              //ph->p_memsz。搜索BSS段
    }
  }
...
void env_run(struct Env *e) {
  ...
  if (curenv != NULL &&
      curenv->env_status ==
          ENV_RUNNING) { //将原来正在运行的Env设置为ENV_RUNNABLE
